#!/usr/bin/env python3
"""Clean OCR historical-record JSON/text into paragraph-flow schema JSON.

Usage:
    python vertical_historical_json_cleaner.py raw_ocr.txt cleaned_schema.json
    python vertical_historical_json_cleaner.py raw_ocr.txt cleaned_schema.json --page-index 0

The output is designed for:
    vertical_historical_docx_generator.py cleaned_schema.json output.docx
...
SCHEMA_VERSION = "vertical_historical_record_paragraph_flow_v1"
...
DOC_NO_RE = re.compile(r"(機密|秘|第[一二三四五六七八九十〇零\d]+[号號]|[甲乙丙丁]号)")
...
def infer_role(text, position, before_marker):
    ...
    if compact in {"記", "左記", "左記", "左　記"}:
        return "marker"
    if ERA_DATE_RE.search(compact):
        return "date"
    if compact.endswith("殿") or compact.startswith("在"):
        return "recipient"
    if any(k in compact for k in ("署長", "警部", "領事", "主任", "書記生")) and before_marker:
        return "sender"
    if DOC_NO_RE.search(compact) and position <= 4:
        return "header"
    if compact in {"甲号", "乙号", "丙", "甲", "乙"}:
        return "header"
    if "件" in compact and before_marker and not compact.startswith("本件"):
        return "title"
    return "body"
...
def build_schema_from_pages(
    ...
    *,
    ...
    source = {
        ...
        "ocr_cache": source_path,
        "notes": [
            "Generated by vertical_historical_json_cleaner.py.",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Clean raw OCR historical-record JSON/text into paragraph-flow schema JSON."
    )
    parser.add_argument("input_raw", help="Raw OCR JSON/txt file")
