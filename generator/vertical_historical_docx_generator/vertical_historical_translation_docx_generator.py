#!/usr/bin/env python3
"""Generate vertical Chinese-translation historical-record DOCX files from paragraph-flow JSON.

Usage:
    python vertical_historical_translation_docx_generator.py input.json output.docx
...
def get_source_cache_key(source):
    ...
    cache_path = source.get("translation_cache") or source.get("ocr_cache")
    if cache_path:
        return Path(str(cache_path)).stem
    return ""
...
def group_translation_pages(pages, max_chars=1050, max_source_pages=3):
    groups = []
    current = []
    current_chars = 0
    for page in pages:
...
def build_doc(json_path_or_schema, out_path, flow_pages=False, merge_pages=True, merge_max_chars=None):
    ...
    if merge_pages and not flow_pages:
        merge_settings = layout.get("translation_merge", {})
        max_chars = merge_max_chars or merge_settings.get("max_chars_per_page", 1050)
        max_source_pages = merge_settings.get("max_source_pages_per_page", 3)
        groups = group_translation_pages(data["pages"], max_chars=max_chars, max_source_pages=max_source_pages)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate a Word/WPS-compatible vertical Chinese-translation historical-record DOCX from JSON."
    )
    ...
    parser.add_argument(
        "--no-merge-pages",
        action="store_true",
        help="Keep one source HRS page per Word page instead of merging translated pages.",
    )
    parser.add_argument(
        "--merge-max-chars",
        type=int,
        help="Approximate maximum translated-character count per merged Word page.",
    )
